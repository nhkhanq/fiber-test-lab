...
- **FNN:** Dockerfile mỏng (`debian:bookworm-slim`) → tải binary official `fnn_v0.8.0-x86_64-linux.tar.gz` → chạy `fnn`. Tự viết entrypoint riêng.

Cả hai đều nguồn official, tự viết compose/Dockerfile → sạch pháp lý, nhanh (không cần compile Rust).
...
2. **Node có tiền CKB** để mở channel — cần một bước cấp vốn (faucet) sau khi devnet lên.
3. **Devnet config** (`config.yml`/`dev.toml`) — official repo chỉ có mainnet/testnet, KHÔNG có devnet sẵn, phải tự ráp.

→ Đây là phần tốn công nhất. Cần tự tái tạo theo hướng **nhúng contracts vào genesis + faucet cấp vốn**, ví dụ:
- **Cách A:** genesis CKB custom tự viết, nhúng sẵn fiber-scripts + bước faucet cấp tiền.
