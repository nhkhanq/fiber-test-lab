...
| **E0-2** Đọc quick-start official | ⬜ chưa | Đã đọc quick-start chính thức, một số nội dung đã có trong ghi chú hands-on khác. |
...
| **E1-1** Check license nguồn tham khảo hạ tầng | ⚠️ ĐÃ KIỂM TRA — RỦI RO | Nguồn tham khảo hạ tầng ban đầu KHÔNG có file LICENSE rõ ràng. "No license" = all-rights-reserved. CẦN human quyết cách xử lý (liên hệ tác giả / reimplement compose / chấp nhận rủi ro hackathon). Chưa ghi decisions-log (chờ confirm). |
| **E1-2** Document compose | ✅ DONE (draft) | Ghi chú hạ tầng compose học tay ở E0, verify khớp thực tế. |
...
| **E6-1** `setupScenario()` + **E6-2** `expectPayment*` | ✅ DONE | **test-kit** (`test-kit/context.ts`, `expect.ts`, `index.ts`; `package.json` exports `./test-kit`). `setupScenario(name)` = up+seed qua `runScenario` (dùng chung với CLI → mọi RPC vẫn log run-log), KHÔNG teardown; trả `ctx {runId, network, scenario, expectation, store, lastPaymentId, lastPaymentFrom, call(node,method,params), reset()}`. `expectPaymentSucceeds(ctx, id?)` poll `get_payment` tới Success (mặc định payment cuối). `expectPaymentFails(ctx, id?, reason?)` xử 2 nhánh: **lỗi đồng bộ** (insufficient-capacity — không có payment_hash) đọc lỗi thô đã ghi; **lỗi bất đồng bộ** poll tới Failed; `reason` (nếu có) map qua `mapError` rồi so. Timeout/mismatch → throw kèm **đường dẫn run-log** (`.fiber-lab/runs/<id>.json`, save trước khi throw — AC "on timeout fails with run-log attached"). Example `test-kit/examples/payments.test.ts` (bare import `fiber-test-lab/test-kit`). Verify LIVE: `vitest run test-kit/examples` PASS 2/2 (122s) — direct-channel expectPaymentSucceeds + insufficient-capacity expectPaymentFails(reason) đúng nhánh lỗi đồng bộ (lastPaymentId null), bare self-import resolve OK. |
| **E6-3** `expectChannelState` + **E6-4** examples/clean-run | ✅ DONE | **E6-3:** `test-kit/expect.ts::expectChannelState(ctx, channelId, {status?, capacity?})` — tìm channel qua `list_channels` mọi node, poll tới khi `state.state_name` khớp `status` (timeout → throw + run-log path). **Phát hiện thật:** `list_channels` KHÔNG có field funding/capacity — chỉ `local_balance`+`remote_balance` (hex shannon, tổng = funding − reserve). Kênh single-funded ⇒ `capacity = (local+remote)/1e8 + MIN_CHANNEL_RESERVE_CKB(99)`. Verify LIVE trên kênh 500: ChannelReady+cap 500 pass; cap 999 & status sai bị reject đúng. **E6-4:** example `test-kit/examples/channel-state.test.ts` + `test-kit/examples/README.md` (prereq: Docker, build FNN lần đầu, cô lập run-id, tự reset). `vitest.config.ts` thêm `fileParallelism:false` (mỗi file dựng 1 cụm docker → chạy tuần tự, giữ tất định BR-DET-001) + `hookTimeout`. Verify LIVE: `npm test` từ clean PASS **5 file / 6 test, 530s** (tuần tự) — determinism + two-hop + insufficient + payments examples + channel-state, docker clean cuối. |
| **E7-1/2/3/5** Docs & submission | ✅ DONE (E7-4 video = human) | **E7-1** `docs/scenario-catalog.md` — pin versions (FNN 0.8.0/335a74a, @ckb-ccc canary 20260330023358, CKB v0.207.0), global config, giới hạn, 3 scenario kèm kết quả verify thật (fee/reason/routeHops), exit codes. **E7-2** `README.md` — problem→solution, prereq (Docker, Node≥20), quick-start CLI + bảng lệnh, scenario YAML mẫu, test-kit usage, link catalog/trade-offs. **E7-3** LICENSE (MIT) đã có sẵn từ đầu repo ✅. **E7-5** `docs/trade-offs.md` — devnet≠mainnet, gossip 2s, peer-offline mô phỏng (docker kill), hop suy ra từ fee, phân loại lỗi bằng substring, single-funded capacity, chi phí thời gian, bảo trì version. Verify: typecheck OK; `npm run lab -- list`/`--help` khớp README (3 scenario, 5 lệnh). **E7-4** (video 3–5 phút) là việc con người — chưa làm. |
| **E8-2** `peer-offline` (stretch) | ✅ DONE | Scenario `topology/scenarios/peer-offline.yaml` (kênh alice→bob, seed `kill_node bob` → send_payment alice→bob). **Phát hiện then chốt:** peer offline cho lỗi `max outbound liquidity 0 is insufficient` — **TRÙNG HỆT message của `insufficient_outbound`** (channel vẫn ChannelReady+enabled nhưng peer offline ⇒ liquidity dùng được = 0). KHÔNG phân biệt được bằng message. Tín hiệu thật: **`list_peers(alice) = []`** (bob bị xoá khỏi peers NGAY khi payment fail). **Impl:** seeder ghi `peerConnected` (query `list_peers`) vào step khi send_payment fail; `errorCategory.ts::classifyFailure()` ưu tiên `peerConnected===false → peer_offline`, còn lại `mapError()`; `verify.ts` verifyReason + test-kit `expectPaymentFails` (nhánh lỗi đồng bộ) dùng `classifyFailure`. Verify LIVE: `up peer-offline` exit 0, `reason {expected:peer_offline, actual:peer_offline, match:true}`; example `test-kit/examples/payments.test.ts` PASS 3/3 (peer-offline + insufficient-capacity KHÔNG regress → vẫn insufficient_outbound). Glossary/notebook/catalog cập nhật. |
| **E8-1** `expired-invoice` (stretch) | ✅ DONE | Scenario `topology/scenarios/expired-invoice.yaml` (new_invoice bob expiry 3s → wait 6s → alice trả invoice đó). **Feature mới — trả invoice:** schema thêm `useInvoice` cho send_payment; seeder `new_invoice` (đổi sang rawCall) bắt `invoice_address` theo node nhận vào map, `send_payment useInvoice:true` gọi `send_payment [{ invoice }]` (không keysend). **Raw data:** `new_invoice`→`{invoice_address:"fibd1...", invoice}`; trả sau hết hạn fail ĐỒNG BỘ `InvalidParameter: Failed to validate payment request: "invoice is expired"` (code -32000) → thêm rule `invoice_expired` vào `mapError`. Verify LIVE: `up expired-invoice` exit 0, `reason {expected:invoice_expired, actual:invoice_expired, match:true}`; example thêm case (payments.test.ts giờ 4 case). Glossary/notebook/catalog cập nhật (4/9 ErrorCategory đã verify: insufficient_outbound, no_route_found, peer_offline, invoice_expired). |
| **E8-3** Event-driven test-kit (subscribe_store_changes) | ✅ DONE | **Verify thật:** method CÓ trong FNN 0.8.0 (system-design đoán 0.8.1) nhưng **chỉ qua WebSocket** (HTTP → `-32603 Internal error`); FNN dùng chung port cho WS ⇒ `ws://<same-endpoint>`. Event `store_changes` → `result.PutPaymentSession.payment_session.status` chuyển Created→Success/Failed. **Impl:** `lib/fiber/subscribe.ts::subscribePayments(endpoint, config)` (dep `ws`+`@types/ws`) — subscribe TRƯỚC khi gửi để không lỡ event, đệm theo hash, `wait(hash)` resolve khi terminal + timeout; test-kit `ctx.watchPayments(node)` + `ctx.endpoints`. Không đụng seeder/poll đang chạy (giữ tất định); event-driven là API bổ sung opt-in. Verify LIVE: example `test-kit/examples/event-driven.test.ts` — subscribe → gửi payment mới → `wait` bắt Success qua event. |
| **E8-4** Multi-asset (RUSD/sUDT) | ✅ DONE | Scenario `multi-asset.yaml` (alice→bob kênh UDT, trả 100 RUSD). **3 phần build thật:** (a) **Trích hằng số genesis** — `simple_udt` code_hash = ckb-blake2b binary = `0xe1e354…` (tất định theo binary FNN v0.8.0 chính thức); cell_dep tx[0]/out[8]; secp256k1 dep_group tx[1]/out[0]. Hằng số tất định theo dev.toml + CKB v0.207.0 (`lib/constants.ts`). (b) **Mint sUDT runtime** `lib/fiber/udt.ts::mintUdt` qua @ckb-ccc: client devnet override secp256k1 dep_group (mặc định CCC là testnet — sai), build tx sUDT (owner = lock hash node), sign, send `passthrough`, chờ commit. CKB expose port map tạm (`compose` + `UpResult.ckbEndpoint`, BR-ISO-002). (c) **FNN `ckb.udt_whitelist`** (nodeConfig, args `0x.*` khớp mọi owner) + **seeder**: mint cho node cấp vốn kênh UDT → open_channel kèm `funding_udt_type_script` → send_payment kèm `udt_type_script`; amount theo asset (CKB ×1e8, UDT thô). Verify LIVE: `up multi-asset` exit 0 — `mint_udt`→`open_channel`(UDT ready)→`send_payment` RUSD **Success**, expect match. Lưu ý: `fiber-lab seed` chạy-lại-riêng chưa hỗ trợ UDT (mint ở `up`). |
...
- Chữ ký RPC chuẩn cho 0.8: xem `rpc-notebook.md`.
