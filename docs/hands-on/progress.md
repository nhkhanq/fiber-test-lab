...
| **E8-4** Multi-asset (RUSD/xUDT) | 🔬 ĐÃ SCOPE — HOÃN | Điều tra thật, KHÔNG implement (quá lớn cho 1 pass + stack #49/#50 chưa merge). **Hiện trạng:** (1) genesis ĐÃ deploy code `simple_udt`+`xudt_rce` (dev.toml system_cells) nhưng `issued_cells` chỉ cấp CKB thuần — **KHÔNG mint UDT token** cho node nào. (2) FNN `udt_cfg_infos` RỖNG (nodeConfig.ts không sinh) ⇒ node chưa nhận diện asset RUSD. (3) seeder CKB-only (open_channel/send_payment không truyền `funding_udt_type_script`). Schema đã có `asset: CKB\|RUSD` sẵn. **Cần 3 phần lớn:** (a) sinh `udt_cfg_infos` (code_hash+args xUDT từ devnet) trong nodeConfig; (b) **mint xUDT** cho 3 key genesis bằng 1 CKB tx qua @ckb-ccc (owner lock, RCE) — không có faucet UDT sẵn; (c) seeder mở channel UDT + trả UDT theo `asset`. Mỗi phần cần reverse-engineer thật (không verify live được kiểu bounded như các ticket khác). **Khuyến nghị:** merge #49/#50 trước, rồi làm E8-4 như effort riêng. |
