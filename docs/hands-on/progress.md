...
| E5-3 → E8 | ⬜ chưa | Kế: E5-3 `insufficient-capacity` (payment fail, reason khớp `insufficient_outbound` — raw error đã có ở E0-5), E5-4 map ErrorCategory (mở rộng verify.ts cho `reason`); rồi E6 test-kit. |
