...
| E8 → | ⬜ chưa (stretch) | Kế (nếu còn thời gian): E8-1 `expired-invoice`, E8-2 `peer-offline` (data đã có từ E3-7: payment sau kill+restart fail "max outbound liquidity 0"), map thêm ErrorCategory (`invoice_expired`/`peer_offline`). |
