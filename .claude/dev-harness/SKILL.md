---
name: dev-harness
description: Autonomous OODA-loop harness for building and fixing Fiber Test Lab features (CLI commands under cli/, core logic under lib/ — fiber RPC client, docker orchestrator, scenario loader/seeder, run-log store —, YAML scenarios under topology/scenarios/, test-kit assertions, global config). Spawns isolated sub-agents for Planner → Implementer → Checker → Eval → PR, sharing state via disk. Use when the user says "build", "add a command", "add a scenario", "fix", "add a helper", or gives any implementation task for the Fiber Test Lab app. Do NOT trigger for pure Q&A, code review, or read-only exploration.
---

# dev-harness — Fiber Test Lab Development Harness
...
**Hard stop overrides `--auto`.** `.context/INDEX.md` and `.context/processes/decisions-log.md`
define the single source of truth. Some categories the project owner must decide
personally: scenario-schema changes (`.context/data-dictionary/scenario-schema.md`),
CLI-spec changes (`.context/api/cli-spec.md`), scenario-rules/business-logic changes
(`.context/business-rules/scenario-rules.md`), adding a new dependency, deleting existing
code/files, and architecture choices with multiple viable approaches. If the Planner's
brief flags any of these in `needs_human_decision`, the orchestrator MUST checkpoint and
wait for an explicit answer before Step 2 — even when `auto = true`. `--auto` only
suppresses routine phase-to-phase checkpoints, never these.
...
- `<task description>`: what to build or fix. Examples: "add a `fiber-lab logs` command",
  "add the two-hop-route scenario", "fix the READY-poll timeout in lib/docker/orchestrator.ts",
  "add expectChannelState to the test-kit".
...
```
This produces e.g. `20260704T192516`. Use it as `<timestamp>`.
...
`feature_type` values: `"cli-command"`, `"lib-core"`, `"scenario"`, `"test-kit"`,
`"config"`, `"docs"`, `"mixed"`. There is no `"api-route"`, `"db-schema"`, or `"contract"`
type — Test Lab has no HTTP server, no database, and never writes CKB Scripts.

`needs_human_decision` entries: `{ "category": "scenario-schema | cli-spec | scenario-rules | new-dependency | deletion | architecture-choice", "detail": "<what's ambiguous and why>" }`

Error object shape:
```json
{ "iteration": 0, "type": "typecheck | lint | test | structural", "severity": "ERROR | WARN", "file": "", "message": "" }
```
...
let the Implementer guess at scenario-schema shape, CLI-spec contract, scenario-rules
logic, which dependency to add, or what to delete — these are the user's call per
`.context/INDEX.md` and `decisions-log.md`. Once resolved, append the resolution to
`<run-dir>/harness-brief.md` under a "Resolved decisions" section before continuing.
